...
###########################################################################
# APPENDIX C: Wire RealtimeDetector -> RYU (Immediate Mitigation Option)
###########################################################################

# Patch notes: RealtimeDetector now can optionally call RYU_BLOCK_ENDPOINT directly to install drop flows
# when a malicious flow is detected. This provides a low-latency mitigation path alongside the SOAR playbook.

# CONFIG: set RYU_BLOCK_ENDPOINT to the Ryu app REST path
RYU_BLOCK_ENDPOINT = 'http://ryu-controller:8080/wm/block'

# Changes to RealtimeDetector.run: when pred==1, call immediate_mitigation if enabled

additional_realtime_patch = r"""
    def immediate_mitigation(self, src_ip, duration=3600):
        try:
            resp = requests.post(RYU_BLOCK_ENDPOINT, json={'src_ip': src_ip, 'duration': duration}, timeout=5)
            if resp.status_code == 200:
                return True, resp.json()
            else:
                return False, {'status_code': resp.status_code, 'text': resp.text}
...
#     self.es.index(...)
#     self.producer.send(...)
#     if getattr(self, 'enable_immediate_mitigation', False):
#         ok, details = self.immediate_mitigation(flow.get('src_ip'))
#         alert['mitigation_immediate'] = {'success': ok, 'details': details}
"""

###########################################################################
# APPENDIX D: Dockerfiles + docker-compose additions
###########################################################################

# File: Dockerfile.detector
dockerfile_detector = r"""
FROM python:3.11-slim
WORKDIR /app
COPY requirements.txt ./
RUN pip install --no-cache-dir -r requirements.txt
COPY soc_siem_soar_reference.py ./
ENV KAFKA_BOOTSTRAP=kafka:9092
ENV ELASTICSEARCH_HOST=http://elasticsearch:9200
ENV RYU_BLOCK_ENDPOINT=http://ryu:8080/wm/block
CMD ["python","soc_siem_soar_reference.py","--run-detector"]
"""

# File: Dockerfile.soar
dockerfile_soar = r"""
FROM python:3.11-slim
WORKDIR /app
COPY requirements.txt ./
...
ENV ELASTICSEARCH_HOST=http://elasticsearch:9200
ENV RYU_BLOCK_ENDPOINT=http://ryu:8080/wm/block
CMD ["python","soc_siem_soar_reference.py","--run-soar"]
"""
...
# Compose additions (append to previous docker-compose.yml) - adds detector, soar, and ryu
docker_compose_add = r"""
  ryu:
    image: osrg/ryu:4.35
    command: ryu-manager --verbose /opt/ryu/app/simple_switch_13.py
    ports:
      - '6633:6633'
      - '8080:8080'
    volumes:
      - ./ryu_sdn_blocker.py:/opt/ryu/app/ryu_sdn_blocker.py
  ...
  detector:
    ...
    environment:
      ...
      - RYU_BLOCK_ENDPOINT=http://ryu:8080/wm/block
...
###########################################################################
# APPENDIX E: Cortex XSOAR Playbook (JSON snippet)
###########################################################################

# This is a simplified XSOAR playbook (as JSON) that:
# 1) Receives an alert (from Kafka connector or manually created incident)
# 2) Enriches via WHOIS / GeoIP
# 3) Calls an integration command to POST to the Ryu controller
# 4) Logs action and closes the incident

cortex_playbook_json = r"""
{
  "name": "Ransomware Network Block",
  "tasks": [
    {
      "id": "1",
      "task": "Get Incident Data",
      "type": "start"
    },
    {
      "id": "2",
    ...
    {
      "id": "4",
      "task": "Block Host via Ryu",
      "type": "integration",
      "integration": "HTTP",
      "args": {
        "url": "http://ryu:8080/wm/block",
        "method": "POST",
        "body": {
    ...
    {
      "id": "5",
      "task": "Add Note & Close Incident",
      "type": "automation",
      "automation": "addNoteToIncident",
      "args": {
        "note": "Blocked IP via Ryu controller"
      }
    }
  ]
}
...
###########################################################################
# APPENDIX F: TheHive Responder / Playbook (YAML-style template)
###########################################################################

thehive_playbook = r"""
# TheHive playbook (case template and responder actions)
name: Ransomware Detection Response
description: When an alert is flagged as ransomware, enrich and block via SDN
steps:
  - name: Enrich alert
    type: enrichment
    action: [whois, geoip, passive-dns]
  - name: Check reputation
    type: decision
    condition: alert.score >= 0.6 or alert.repeat_count > 2
    actions:
      - name: Create case
        type: createCase
      - name: Run responder - block via Ryu
        type: responder
        responder: http-request
        args:
          method: POST
          url: http://ryu:8080/wm/block
          body: '{"src_ip":"{{artifact.ip}}","duration":3600}'
  - name: Close case
    type: finalize
"""

###########################################################################
# APPENDIX G: Feature extractor additions for NetFlow v5 CSV and Zeek conn.log
###########################################################################

feature_extensions = r"""
# NetFlow v5 CSV expected columns (example):
# 'unix_secs','unix_nsecs','flow_seq','srcaddr','dstaddr','nexthop','input','output',
# 'dPkts','dOctets','first','last','srcport','dstport','tcp_flags','prot','tos','src_as','dst_as','src_mask','dst_mask'

def extract_netflow_v5_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # timestamps
    df['first_ts'] = pd.to_datetime(df['first'], unit='s', errors='coerce')
    df['last_ts'] = pd.to_datetime(df['last'], unit='s', errors='coerce')
    df['duration'] = (df['last_ts'] - df['first_ts']).dt.total_seconds().clip(lower=0.000001)
    df['packets'] = df['dPkts']
    df['bytes'] = df['dOctets']
    df['bytes_per_packet'] = df['bytes'] / df['packets'].replace(0,1)
    df['pkt_rate'] = df['packets'] / df['duration']
    df['byte_rate'] = df['bytes'] / df['duration']
    df['protocol_num'] = df['prot'].astype(int)
    df['dst_port_is_high'] = (df['dstport'] > 1024).astype(int)
    df['src_ip_hash'] = df['srcaddr'].apply(lambda x: hash(x) % (2**16))
    df['dst_ip_hash'] = df['dstaddr'].apply(lambda x: hash(x) % (2**16))
    features = ['duration','bytes_per_packet','pkt_rate','byte_rate','dst_port_is_high','protocol_num','src_ip_hash','dst_ip_hash']
    return df[features]

# Zeek conn.log expected columns (TS, uid, id.orig_h, id.orig_p, id.resp_h, id.resp_p, proto, service, duration, orig_bytes, resp_bytes, conn_state)

def extract_zeek_conn_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df['duration'] = df['duration'].astype(float).clip(lower=0.000001)
    df['packets'] = (df['orig_pkts'].fillna(0) + df['resp_pkts'].fillna(0)).replace(0,1)
    df['bytes'] = (df['orig_bytes'].fillna(0) + df['resp_bytes'].fillna(0)).replace(0,0)
    df['bytes_per_packet'] = df['bytes'] / df['packets']
    df['pkt_rate'] = df['packets'] / df['duration']
    df['byte_rate'] = df['bytes'] / df['duration']
    df['protocol_num'] = df['proto'].map({'tcp':6,'udp':17}).fillna(0).astype(int)
    df['dst_port_is_high'] = (df['id.resp_p'] > 1024).astype(int)
    df['src_ip_hash'] = df['id.orig_h'].apply(lambda x: hash(x) % (2**16))
    df['dst_ip_hash'] = df['id.resp_h'].apply(lambda x: hash(x) % (2**16))
    features = ['duration','bytes_per_packet','pkt_rate','byte_rate','dst_port_is_high','protocol_num','src_ip_hash','dst_ip_hash']
    return df[features]
"""

###########################################################################
# Append all new artifacts to canvas
###########################################################################
